...
void TestHelixFit(unsigned ntries, unsigned maxniter=10, double reffac=3.0, double momval=105.0,double charge=1.0,double genmass=0.5,double fitmass=0.5,double deltaz=75.0,double sigdrift=0.2, double sigtime=0.3, bool invert=false) {
  ...
  parsig[_rad] = 0.5*scale; // approximate error in radius 
  parsig[_cx]= 0.5*scale; // center x
  parsig[_cy]= 0.5*scale; // in center y
  parsig[_lambda] = 0.1*scale; // in tan lambda
  parsig[_phi0]= 0.005*scale; // in phi0
  parsig[_t0]= 5.0*scale; // in t0
...
// momentum plots
  ...
  for(unsigned itry = 0; itry < ntries; itry++){
    ...
    genpars._pars[_t0] = gRandom->Uniform(-10.0,10.0);
    // generate hits
    std::vector<DHit> hits;
    GenerateDriftHits(genpars,hits,nz,deltaz,sigdrift);
//    cout << "nz " << nz << " nhits " << hits.size() << endl;
//    cout << "generator pars " << endl << genpars._pars << endl;
    THit thit;
    thit._sigt = sigtime;
    ...
    for(unsigned ipar=0;ipar < _npars; ipar++){
      ...
      refpars._pars[ipar] += gRandom->Gaus(0.0,parsig[ipar]*reffac);
    }
    if(invert)refpars.invert();
    PVec fpar = refpars._pars;
    PMat fcov = cov;
...
//      cout << "N hits = " << hits.size() << endl;
    //  cout << "true parameters          " << pars._pars << endl;
    //  cout << "fit reference parameters " << refpars._pars << endl;
    ...
    for(size_t ipar=0;ipar< _npars; ipar++){
//      dpref[ipar]->SetPoint(itry, refpars._pars[ipar], fpar[ipar]);
      dpgen[ipar]->SetPoint(itry, genpars._pars[ipar], fpar[ipar]);
//      dprefh[ipar]->Fill(fpar[ipar]-refpars._pars[ipar]);
      dpgenh[ipar]->Fill(fpar[ipar]-genpars._pars[ipar]);
//      dpullrefh[ipar]->Fill((fpar[ipar]-refpars._pars[ipar])/cdiag[ipar]);
      dpullgenh[ipar]->Fill((fpar[ipar]-genpars._pars[ipar])/cdiag[ipar]);
...
// accumulate chisquared info
    ...
    momproj[_rad] = factor*refpars._pars[_rad]/refpars.pbar();
    momproj[_lambda] = factor*refpars._pars[_lambda]/refpars.pbar();
    double momerr2 = Similarity(fcov,momproj);
//    double mommag = fabs(factor)*sqrt(fpar[_rad]*fpar[_rad] + fpar[_lambda]*fpar[_lambda] );
    FourV fmom;
    HelixMom(refpars,fpar[_t0],fmom);
